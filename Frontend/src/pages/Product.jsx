...
const Product = () => {
  ...
  useEffect(() => {
    ...
    if (foundProduct) {
      ...
      // ✅ Fix: use image URL instead of full object
      if (Array.isArray(foundProduct.image) && foundProduct.image.length > 0) {
        setImage(foundProduct.image[0].url);
      }
    }
  }, [productId, products]);
        ...
        <div className="flex w-auto gap-4">
          <div className="flex flex-col gap-2">
            {Array.isArray(productData.image) &&
              productData.image.map((item, index) => (
                <img
                  key={index}
                  src={item.url} // ✅ Fixed here
                  alt={`Thumbnail ${index}`}
                  className={`w-20 h-20 object-cover rounded-sm border cursor-pointer hover:border-black ${
                    image === item.url ? "border-black" : ""
                  }`}
                  onClick={() => setImage(item.url)} // ✅ Fix here
                />
              ))}
          </div>
          ...
          <div className="w-96">
            <img
              ...
              src={image} // ✅ Correct image display
      ...
      <div className="mt-20">
        ...
        <div className="flex flex-col gap-4 border px-6 py-6 text-sm text-gray-500">
          <p>
            ...
            aliquid animi doloribus veritatis ea, accusantium obcaecati, aperiam
            quaerat rerum quasi similique voluptatibus? Sapiente incidunt animi
            quod omnis labore commodi est.
          </p>
          <p>
            Lorem ipsum dolor sit amet consectetur adipisicing elit. Cumque,
            reiciendis aliquid hic recusandae soluta dicta quas libero. Nam
            voluptates distinctio necessitatibus commodi? Atque quibusdam
            blanditiis minima accusantium rem illum perferendis.
          </p>
        </div>
      </div>
